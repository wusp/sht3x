...
import com.maxtropy.ilaundry.monitor.roc.message.send.ReserveResponse;
...
public class SerialService implements SerialResponseListener {
    ...
    public void changeReserveState(ReserveRequest request) {
        ...
        reserveResponse.setTopic(request.getTopic());
        if (request.getReserveState() == 1) {
            ...
            if (status != Status.idle && TextUtils.equals(config.getOrderId(), Const.cardWashOrderId)) {
                roc.sendMessage(new ReservableStatusMessage(ReservableStatusMessage.Status.card_reader_reserved));
                reserveResponse.setReserved(false);
                reserveResponse.setStatus(status.value);
                Roc.getInstance().sendMessage(reserveResponse);
                return;
            ...
            if (this.status == Status.idle && !TextUtils.equals(request.getOrderId(), config.getOrderId())) {
                ...
                reserveResponse.setReserved(true);
                reserveResponse.setStatus(this.status.value);
                Roc.getInstance().sendMessage(reserveResponse);
            }
        } else {
            //取消预约
    ...
    public void reportReserveStatus() {
        ...
        switch (this.status) {
            case idle:
                reserveStatus = ReservableStatusMessage.Status.available;
                break;
            case reserved:
                reserveStatus = ReservableStatusMessage.Status.reserved_deprecated;
                break;
            case paid:
                reserveStatus = ReservableStatusMessage.Status.in_use;
                break;
            case started:
                reserveStatus = ReservableStatusMessage.Status.machine_running;
                break;
            case done:
                reserveStatus = ReservableStatusMessage.Status.available;
                break;
            case error:
                reserveStatus = ReservableStatusMessage.Status.error;
                break;
            case initialization:
                reserveStatus = ReservableStatusMessage.Status.offline;
                break;
            default:
                reserveStatus = null;
        }
